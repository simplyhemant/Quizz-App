...
async function fetchQuestions() {
    try {
        ...
        const allQuestions = await response.json(); // Assume API returns an array of question objects
        
          // Filter only Python questions (IDs 1-10)
          questions = allQuestions.filter(q => q.id >= 1 && q.id <= 10);
